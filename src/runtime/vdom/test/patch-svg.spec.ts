...
describe('renderer', () => {
  ...
  describe('created trailing svg element', () => {
    it('should not affect subsequent created element', () => {
      commitDOMModifications(patch(vnode0, h('div', null, h('div', null, h('svg', null)))));

      const vnode1 = toVNode(vnode0.$elm$);

      commitDOMModifications(patch(vnode1, h('div', null, [h('div', null, h('svg', null)), h('div', null)] as any)));
