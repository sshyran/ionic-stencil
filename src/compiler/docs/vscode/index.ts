...
import { OutputTarget, OutputTargetDocsVscode } from '../../output-targets';
...
export const generateVscodeDocs = async (
  ...
  const vsCoOutputTargets = outputTargets.filter(isOutputTargetDocsVscode);
  if (vsCoOutputTargets.length === 0) {
    return;
  }

  await Promise.all(
    vsCoOutputTargets.map(async (outputTarget: OutputTargetDocsVscode): Promise<void> => {
      const json = {
        /**
